...
Doc format quirks handled here:
  ...
  - The Workplace doc doesn't use "N-M." tags at all: each question is
    ...
    tag mapping.
  - The Impromptu ("0. 돌발") doc groups its questions by named category
...
HEADER_RE = re.compile(r"^([A-Za-z0-9]+-\d+)\.?\s*<", re.MULTILINE)
...
def main():
    ...
    def sort_key(t):
        prefix, _, num = t.rpartition("-")
        return (prefix, int(num) if num.isdigit() else num)

    for tag in sorted(scripts, key=sort_key):
